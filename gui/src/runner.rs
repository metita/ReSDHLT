...
pub enum Msg {
    ...
    /// The whole run finished: total seconds and an explicit outcome.
    Finished(f64, RunOutcome),
}
...
impl Job {
    ...
    /// Used while the application is closing so the compiler cannot be left
    /// running after the GUI disappears.
...
fn tool_path(tools_dir: &str, exe: &str) -> Option<PathBuf> {
    let dir = Path::new(tools_dir.trim());
...
/// Immutable snapshot consumed by the worker. Preview and execution are both
/// generated by the same command builder, so edits made while a job is running
/// can never alter its targets or arguments.
#[derive(Debug, Clone)]
...
fn stage_enabled(opts: &Options, stage: Stage) -> bool {
    match stage {
        Stage::Csg => opts.run_csg,
        Stage::Bsp => opts.run_bsp,
        Stage::Vis => opts.run_vis,
        Stage::Rad => opts.run_rad,
    }
}
...
fn predicted_map(opts: &Options) -> PathBuf {
    ...
    opts.work_dir()
        .and_then(|dir| source.file_name().map(|name| dir.join(name)))
        .unwrap_or(source)
}
...
fn commands_for(opts: &Options, map: &Path) -> Vec<PlannedCommand> {
    ...
    STAGES
        .iter()
        .copied()
        .filter(|stage| stage_enabled(opts, *stage))
        .map(|stage| {
            let exe = tool_path(&opts.tools_dir, stage.exe())
                .unwrap_or_else(|| Path::new(opts.tools_dir.trim()).join(stage.exe()));
...
/// Blocking problems checked both by the button state and immediately before
/// starting. The second check closes the race where a file disappears between
/// UI refreshes.
pub fn validation_errors(opts: &Options) -> Vec<String> {
    let mut errors = opts.extra_args_errors();
    if !STAGES.iter().any(|stage| stage_enabled(opts, *stage)) {
        errors.push("Activa al menos una etapa (CSG, BSP, VIS o RAD).".to_string());
    }

    let map = Path::new(opts.map_path.trim());
    if opts.map_path.trim().is_empty() || !map.is_file() {
        errors.push("Selecciona un archivo .map válido.".to_string());
    } else if !map
        .extension()
        ...
        .unwrap_or(false)
    {
        errors.push("El archivo fuente debe tener extensión .map.".to_string());
    }

    for stage in STAGES.iter().copied().filter(|s| stage_enabled(opts, *s)) {
        if tool_path(&opts.tools_dir, stage.exe()).is_none() {
            errors.push(format!(
                "Falta {} en la carpeta de herramientas.",
                stage.exe()
            ));
        }
    }

    if opts.run_csg && !opts.run_bsp && (opts.run_vis || opts.run_rad) {
        errors.push(
            "CSG vuelve a generar el BSP base: activa BSP antes de ejecutar VIS o RAD.".to_string(),
        );
    }
    if (opts.run_csg || opts.run_bsp) && !opts.run_vis && opts.run_rad {
        errors.push(
            "CSG/BSP invalidan la visibilidad anterior: activa VIS antes de RAD.".to_string(),
        );
    }

    let work_map = predicted_map(opts);
    let base = work_map.with_extension("");
    let finished_bsp = opts.output_base().map(|dir| {
        dir.join(
            work_map
                .with_extension("bsp")
                .file_name()
    ...
    if opts.run_bsp && !opts.run_csg {
        ...
        if !missing.is_empty() {
            errors.push(format!(
                "Para saltar CSG y ejecutar BSP faltan estos intermedios: {}.",
                missing.join(", ")
            ));
        }
    }
    if opts.run_vis && !opts.run_bsp {
        let mut missing = Vec::new();
        if !bsp_available {
            missing.push(".bsp");
        }
        ...
        if !missing.is_empty() {
            errors.push(format!(
                "Para saltar BSP y ejecutar VIS faltan: {}.",
                missing.join(", ")
            ));
        }
    }
    if opts.run_rad && !opts.run_bsp && !opts.run_vis && !bsp_available {
        errors.push("Para ejecutar solo RAD falta el .bsp anterior.".to_string());
    }
    errors
}
...
impl CompilePlan {
    ...
    pub fn preview(opts: &Options) -> String {
        ...
        for command in commands {
            ...
            if command.stage == Stage::Csg && normalized.will_resolve_wads() {
                out.push_str("  # + -wadcfgfile resdhlt-wads.cfg si se resuelven WADs");
            }
            out.push('\n');
        }
        out
...
impl PlannedCommand {
    fn display(&self) -> String {
        let mut parts = vec![quote_arg(&self.exe.display().to_string())];
        parts.extend(self.args.iter().map(|arg| quote_arg(arg)));
        parts.push(quote_arg(&self.target.display().to_string()));
        let command = parts.join(" ");
        match &self.work_dir {
            Some(dir) => format!("[{}] {command}", dir.display()),
            None => command,
        }
    }
}
...
fn kill_process_tree(child: &mut std::process::Child) {
    // Some tools may start helpers. `Child::kill` only terminates the direct
    // process on Windows, while taskkill /T also removes its descendants.
...
fn acquire_workspace_lock(opts: &Options) -> Result<WorkspaceLock, String> {
    let source = PathBuf::from(opts.map_path.trim());
    let lock_path = if let Some(work_dir) = opts.work_dir() {
        std::fs::create_dir_all(&work_dir).map_err(|error| {
            format!("no pude crear el workspace {}: {error}", work_dir.display())
        })?;
        work_dir.join(".resdhlt.compile.lock")
    } else {
        let parent = source.parent().ok_or("el mapa no tiene carpeta padre")?;
        let stem = source
            .file_stem()
            .and_then(|value| value.to_str())
            .unwrap_or("mapa");
        parent.join(format!(".{stem}.resdhlt.compile.lock"))
    };
    ...
    let mut file = OpenOptions::new()
        ...
        .write(true)
        .open(&lock_path)
        .map_err(|error| format!("no pude abrir el lock {}: {error}", lock_path.display()))?;
    file.try_lock_exclusive().map_err(|_| {
        format!(
            "ya hay otra compilación usando este workspace: {}",
            lock_path.display()
        )
    })?;
    file.set_len(0)
        .and_then(|_| writeln!(file, "pid={}", std::process::id()))
        .map_err(|error| format!("no pude escribir el lock {}: {error}", lock_path.display()))?;
...
fn prepare_workspace(opts: &Options, tx: &Sender<Msg>) -> Option<PathBuf> {
    let src = PathBuf::from(opts.map_path.trim());
    if !src.is_file() {
        let _ = tx.send(Msg::Line(
            LineKind::Error,
    ...
    let same = std::fs::canonicalize(&src).ok() == std::fs::canonicalize(&dst).ok() && dst.exists();
    if same {
        return Some(dst);
    }
...
/// Finished BSPs are moved out of `intermedios` after a successful run. When a
/// later run starts at BSP, VIS or RAD, seed that workspace with the previous
/// result so skipping an early stage works as advertised.
fn seed_previous_bsp(opts: &Options, map: &Path) -> Result<(), String> {
    if opts.run_csg {
        return Ok(());
    }
    let destination = map.with_extension("bsp");
    if destination.is_file() {
        return Ok(());
    }
    let Some(output) = opts.output_base() else {
        return Ok(());
    };
    let source = output.join(destination.file_name().unwrap_or_default());
    ...
    std::fs::copy(&source, &destination)
        ...
        .map_err(|error| {
            format!(
                "no pude preparar el BSP anterior {} en {}: {error}",
                source.display(),
                destination.display()
            )
        })
...
pub fn start(plan: CompilePlan) -> Job {
    let (tx, rx) = channel();
    ...
    let worker = thread::spawn(move || {
        ...
        let _workspace_lock = match acquire_workspace_lock(&opts) {
            Ok(lock) => lock,
            Err(error) => {
                let _ = tx.send(Msg::Line(LineKind::Error, error));
        ...
        if let Err(error) = seed_previous_bsp(&opts, &map) {
            let _ = tx.send(Msg::Line(LineKind::Error, error));
            let _ = tx.send(Msg::Finished(
                overall.elapsed().as_secs_f64(),
                RunOutcome::Failed,
        ...
        if opts.auto_wads && opts.run_csg {
            ...
            if wads.is_empty() {
                let _ = tx.send(Msg::Line(
                    LineKind::Warning,
                    "No pude resolver ningún .wad; se usa la lista del mapa tal cual.".to_string(),
                ));
        ...
        // Rebuild only to account for the canonical workspace returned after
        // its directories and source copy were prepared. It uses the exact
        // same builder as `CompilePlan::new` and `preview`.
        ...
        for command in commands {
            ...
            let _ = tx.send(Msg::Line(
                ...
                format!(
                    "{} {} {}",
                    quote_arg(&command.exe.display().to_string()),
                    args.iter()
                        .map(|a| quote_arg(a))
                        .collect::<Vec<_>>()
                        .join(" "),
                    quote_arg(&command.target.display().to_string())
                ),
            ));
            ...
            let mut child = match cmd.spawn() {
                ...
                Err(e) => {
                    let _ = tx.send(Msg::Line(
                        LineKind::Error,
                        format!("No pude ejecutar {}: {e}", command.exe.display()),
                    ));
                    all_ok = false;
                    break;
                }
            ...
            if !ok {
                ...
                if cancelled {
                    let _ = tx.send(Msg::Line(
                        LineKind::Warning,
                        format!("{} cancelado por el usuario.", stage.name()),
                    ));
                } else {
                    let _ = tx.send(Msg::Line(
                        LineKind::Error,
        ...
        let _ = tx.send(Msg::Finished(overall.elapsed().as_secs_f64(), outcome));
    });
...
mod tests {
    ...
    #[test]
    fn validation_rejects_an_empty_pipeline_and_missing_tools() {
        let mut options = Options::default();
        options.run_csg = false;
        options.run_bsp = false;
        options.run_vis = false;
        options.run_rad = false;
        let errors = validation_errors(&options);
        assert!(errors
            .iter()
            .any(|error| error.contains("al menos una etapa")));

        options.run_rad = true;
        options.tools_dir = std::env::temp_dir()
            .join("resdhlt-no-tools")
            .display()
            .to_string();
        let errors = validation_errors(&options);
        assert!(errors.iter().any(|error| error.contains("sdHLRAD")));
    }

    #[test]
    fn plan_preview_uses_real_stage_targets_and_quotes_paths() {
        let root = std::env::temp_dir().join(format!("resdhlt plan test {}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        let tools = root.join("tools");
        let output = root.join("output");
        std::fs::create_dir_all(&tools).unwrap();
        std::fs::create_dir_all(&output).unwrap();
        let map = root.join("map with spaces.map");
        std::fs::write(&map, "{}").unwrap();
        for stage in STAGES {
            std::fs::write(tools.join(format!("{}.exe", stage.exe())), "").unwrap();
        }

        let mut options = Options::default();
        options.map_path = format!("  {}  ", map.display());
        options.tools_dir = tools.display().to_string();
        options.output_dir = output.display().to_string();
    ...
    #[test]
    fn display_quoting_handles_spaces_quotes_and_trailing_slashes() {
        assert_eq!(quote_arg("plain"), "plain");
        assert_eq!(quote_arg("two words"), "\"two words\"");
        assert_eq!(quote_arg(""), "\"\"");
        assert_eq!(
            quote_arg("C:\\folder with space\\"),
            "\"C:\\folder with space\\\\\""
        );
    }

    #[test]
    fn rad_only_reuses_the_finished_bsp_from_the_project_folder() {
        let root =
            std::env::temp_dir().join(format!("resdhlt-rad-only-plan-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        let tools = root.join("tools");
        ...
        std::fs::write(tools.join("sdHLRAD.exe"), "").unwrap();
        let map = source.join("night map.map");
        std::fs::write(&map, "{}").unwrap();

        let options = Options {
            map_path: map.display().to_string(),
            tools_dir: tools.display().to_string(),
            output_dir: output.display().to_string(),
            project_name: "night".to_string(),
            run_csg: false,
            run_bsp: false,
            run_vis: false,
            run_rad: true,
        ...
        let finished = options.output_base().unwrap().join("night map.bsp");
        std::fs::create_dir_all(finished.parent().unwrap()).unwrap();
        std::fs::write(&finished, "bsp").unwrap();
        assert!(validation_errors(&options).is_empty());

        let work_map = prepare_workspace(&options, &channel().0).unwrap();
        seed_previous_bsp(&options, &work_map).unwrap();
        assert_eq!(
            std::fs::read_to_string(work_map.with_extension("bsp")).unwrap(),
            "bsp"
        );
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    #[test]
    fn only_one_compile_can_own_a_workspace() {
        let root =
            std::env::temp_dir().join(format!("resdhlt-workspace-lock-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).unwrap();
        let map = root.join("locked.map");
        std::fs::write(&map, "{}").unwrap();
        let options = Options {
            map_path: map.display().to_string(),
        ...
        let first = acquire_workspace_lock(&options).unwrap();
        let error = acquire_workspace_lock(&options).unwrap_err();
        assert!(error.contains("otra compilación"), "{error}");
        drop(first);
        assert!(acquire_workspace_lock(&options).is_ok());
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    fn relocates_wads_from_another_machine() {
        ...
        assert!(
            !names.contains(&"zgaminglogos.wad".to_string()),
            "{names:?}"
        );
