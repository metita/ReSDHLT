...
bool			g_allleaks = false;					// survey every hole, not just the first one found
...
static void     Usage()
{
	...
	Log("    -allleaks      : On a leak, survey the map and mark every hole, not just one\n");
...
static void     Settings()
{
	...
	Log("survey all leaks    [ %7s ] [ %7s ]\n", g_allleaks? "on": "off", "off");
...
#define g_dplanes g_mapplanes
    ...
    // every hull has been filled by now, so the leak - if there was one - can
    // be reported once instead of per hull
    PrintLeakSummary();
...
int             main(const int argc_input, char** argv_input)
{
    ...
    for (i = 1; i < argc; i++)
    {
		...
		else if (!strcasecmp(argv[i], "-allleaks"))
		{
			g_allleaks = true;
		}
